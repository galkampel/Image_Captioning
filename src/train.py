...
class Trainer:
    def __init__(self, criterion, device, vocab_size, enc_dim=2048, dec_dim=512, ** models_params):
        ...
        # seq2seq params: 'teacher_force_ratio'= 1.0

    def set_optimizer(self, optimizer_params):
        lr = optimizer_params.get('lr', 1e-3)
        wd = optimizer_params.get('wd', 0.0)
        component_name = optimizer_params.get('component', 'decoder')
        optimizer_func = optimizer_params.get('optimizer', Adam)
        optimizer = None
    ...
    def train(self, train_loader, optimizer_dec, optimizer_enc=None):  # return the average training loss of an epoch
    ...
    def evaluate_example(self, img, references, vocab, beam_search_params):  # evaluate one example (bleu score)
        ...
        caption_pred, alphas, pred_val = self.model.predict(img, vocab, k=beam_search_params)
    ...
    def load_model(self, model_name, folder_checkpoint, epoch, optimizer_dec, optimizer_enc=None):
        file_name = f'{model_name}_epoch={epoch}'
        full_path = os.path.join(folder_checkpoint, f'{file_name}.pth')
    ...
    def save_model(self, epoch, folder_checkpoint, optimizer_dec, model_name='Resnet_Lstm', optimizer_enc=None):
        model_saved_name = model_name + f'_epoch={epoch}'
        full_path = os.path.join(folder_checkpoint, f'{model_saved_name}.pth')
