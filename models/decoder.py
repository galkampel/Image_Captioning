...
class Attention(nn.Module):
    def __init__(self, encoder_dim, decoder_dim, attn_type='basic', attn_dim=256):  # can apply basic if enc_dim=dec_dim
        ...
        if self.attn_type == 'multiplicative':
            ...
            self.W_out = nn.Linear(attn_dim, 1)
    ...
    def set_attn_score(self, enc_hiddens, dec_prev_hidden):
        '''
        Args:
            enc_hiddens: encoder's features (BXnum_pixelsXencoder_hidden_size)
            dec_prev_hidden: decoder's previous (first) hidden layer (BXdecoder_hidden_size)

        Returns:
            score: attention score  # (BXnum_pixels)
        '''
        scores = None
        if self.attn_type == 'multiplicative':  # s^T* W * h_i , s- decoder, h- encoder
            h_prev_projected = self.W(dec_prev_hidden).unsqueeze(-1)  # (BXenc_dimX1)
            scores = torch.matmul(enc_hiddens, h_prev_projected).squeeze(-1)  # (BXnum_pixelsX1) -> (BXnum_pixels)
        elif self.attn_type == 'additive':  # tanh(W_1h_i + W_2s ) W_out^T
            h_prev_projected = self.W1(dec_prev_hidden)  # (BXattn_dim)
            enc_hiddens_projected = self.W2(enc_hiddens)  # (BXnum_pixelsXattn_dim)
            scores = torch.tanh(h_prev_projected.unsqueeze(1) + enc_hiddens_projected)  # (BXnum_pixelsXattn_dim)
            scores = self.W_out(scores).squeeze(-1)  # (BXnum_pixels)
        elif self.attn_type == 'basic':
            scores = torch.matmul(enc_hiddens, dec_prev_hidden.unsqueeze(1)).squeeze(-1)  # (BXnum_pixels)
        return scores

    def forward(self, enc_hiddens, dec_prev_hidden):
        '''
        Args:
            enc_hiddens: encoder's hidden states (features)  (BXnum_pixelsXenc_dim)
            dec_prev_hidden: decoder previous hidden state (BXhidden_size)/(BX(num_layers*num_directions)Xhidden_size)
            h_n.view(num_layers, num_directions, batch, hidden_size)
        Returns:
            context vector (BXenc_dim) and attention weights (BXnum_pixels)
        '''
        attn_score = self.set_attn_score(enc_hiddens, dec_prev_hidden)  # (BXnum_pixels)
        attn_weights = F.softmax(attn_score, dim=1)  # (BXnum_pixels)
        context_vectors = (attn_weights.unsqueeze(-1) * enc_hiddens).sum(axis=1).unsqueez(1)  # (BXseq_len=1Xenc_dim)
